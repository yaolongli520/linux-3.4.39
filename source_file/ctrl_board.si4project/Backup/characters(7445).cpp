...
int show_free_type(wchar_t *wtext, int size, PIXEL_TYPE color, int start_x, int start_y)
{
	...
	uint8_t *clr_start = &show_buf[(start_y + 16)* xres * bpp + start_x * bpp];
	int row  = wcslen(wtext) * 17; //一个字擦除多少列

	printf("(start_y + 16)* xres * bpp + start_x * bpp =%d \n",(start_y + 16)* xres * bpp + start_x * bpp);
	for(int i = 0; i <32 ; i++) {
		memset(clr_start, 0, row*bpp);
		clr_start += xres * bpp;
	}
